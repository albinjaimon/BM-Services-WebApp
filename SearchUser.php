...
echo "<p><a href=\"view_users2.php\">Return to all users</a></p>";
...
<?php 
	...
	if ($queryResultReords >0) {
		while ($row = mysqli_fetch_assoc($result)) {
            //data output class
            echo "<h1>$userField: $criteria</h1>";

			echo "<div class ='dataoutput'>
			<h2>User ID: ".$row['user_id']."</h2>
			<h3>Forename: ".$row['first_name']."</h3>
			<h3>Surname: ".$row['last_name']."</h3>
            <h3>Email: ".$row['email']."</h3>
            <h3>Telephone Num: ".$row['user_telephone']."</h3>
            <h3>Home Address: ".$row['user_address']."</h3>
			</div>";
		} 
    } else {
		echo "No records";
